...
class Hyperparameters:
    ...
    # Test-time training (LoRA TTT) at evaluation
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        if rank == 0 and (bi // batch_size) % 10 == 0:
            ...
            if token_count.item() > 0:
                running_bpb = (loss_sum.item() / math.log(2.0)) / byte_sum.item()
            print(f"  ttt_eval [{pct:5.1f}%] {done}/{len(rank_docs)} docs running_bpb={running_bpb:.6f}", flush=True)
